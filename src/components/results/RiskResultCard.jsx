...
export default function RiskResultCard({ assessment }) {
  ...
  const { risk_classification, lipid_targets, alerts, derived_values, moulin_score, triglycerides } = assessment;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Escore de Moulin (SQF) */}
      {moulin_score && moulin_score.applicable && (
        <Card className="border-2 border-purple-200 bg-gradient-to-br from-purple-50 to-indigo-50">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">Escore Total</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-bold text-purple-700">
                    {moulin_score.score}
                  </span>
                  <span className="text-lg text-gray-600">pontos</span>
                </div>
              </div>
              <Badge
                className={`text-lg px-4 py-2 ${
                  moulin_score.interpretation === "muito_provável"
                    ? "bg-red-500 text-white"
                    : moulin_score.interpretation === "improvável"
                    ? "bg-yellow-500 text-white"
                    : "bg-gray-400 text-white"
                }`}
              >
                {moulin_score.interpretationLabel}
              </Badge>
            </div>
            ...
            <div className="bg-white rounded-lg p-4 space-y-2">
              ...
              {moulin_score.criteriaMet && moulin_score.criteriaMet.length > 0 ? (
                <ul className="space-y-1">
                  {moulin_score.criteriaMet.map((criterion, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-sm">
                      <CheckCircle2 className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />
                      <span>
                        <span className="font-medium">{criterion.description}</span>
                        <span className="text-gray-500 ml-2">
                          (+{criterion.points > 0 ? criterion.points : criterion.points} pontos)
                        </span>
                      </span>
                    </li>
                  ))}
                ...
                <p className="text-sm text-gray-500">Nenhum critério atendido</p>
              )}
            </div>

            {moulin_score.criteriaNotMet && moulin_score.criteriaNotMet.length > 0 && (
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                <p className="text-sm font-semibold text-gray-700 mb-2">
                  Critérios Não Atendidos:
                </p>
                <ul className="space-y-1">
                  {moulin_score.criteriaNotMet.slice(0, 5).map((criterion, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-sm">
                      <Info className="w-4 h-4 text-gray-400 mt-0.5 flex-shrink-0" />
                      <span className="text-gray-600">
                        {criterion.description}
                        {criterion.current !== undefined && (
                          <span className="text-gray-500 ml-2">
                            (atual: {typeof criterion.current === 'object' 
                              ? criterion.current.join(', ') 
                              : criterion.current})
                          </span>
                        )}
                      </span>
